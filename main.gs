  ...
  Timesheets.prototype.actionSignIn = function(username, message, channel) {
    if(this.datetime) {
      ...
      if(!data.signIn || data.signIn === '-') {
        ...
        if(!!this.time) {
          // 既存の時間と異なる場合のみ更新
          if(!_.isDate(data.signIn) || data.signIn.getTime() !== this.datetime.getTime()) {
            this.storage.set(username, this.datetime, {signIn: this.datetime});
  ...
  Timesheets.prototype.actionSignOut = function(username, message, channel) {
    if(this.datetime) {
      ...
      if(!data.signOut || data.signOut === '-') {
        ...
        if(!!this.time) {
          // 既存の時間と異なる場合のみ更新
          if(!_.isDate(data.signOut) || data.signOut.getTime() !== this.datetime.getTime()) {
            this.storage.set(username, this.datetime, {signOut: this.datetime});
