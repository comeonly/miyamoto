...
var init = function() {
  ...
  if(spreadsheetId) {
    ...
    var slack = new Slack(token, template, settings);
...
function doPost(e) {
  ...
  // Bot自身の投稿は無視
  if (data.event.subtype === 'bot_message' || data.event.bot_id) {
    return ContentService.createTextOutput('ignored');
  }

  // miyamoto初期化＆打刻処理
  var miyamoto = init();
  miyamoto.receiver.receiveMessage(data); 
...
// Slack = loadSlack();
loadSlack = function () {
  var Slack = function(token, template, settings) {
  ...
  Slack.prototype.receiveMessage = function(data) {
    ...
    var username = json.ok ? json.user.name : userId;

    // 無視ユーザー判定
    var ignore_users = (this.settings.get("無視するユーザ") || '')
      .toLowerCase().replace(/^\s*(.*?)\s*$/, "$1").split(/\s*,\s*/);
    ...
    // 打刻処理
    this.fireEvent('receiveMessage', username, body, channel);
  };
  ...
  // メッセージ送信
  Slack.prototype.send = function(channel, message) {
  ...
  // テンプレート付きでメッセージ送信
  Slack.prototype.template = function(channel) {
    var args = Array.prototype.slice.call(arguments, 1); // channel以外を抽出
    var message = this._template.template.apply(this._template, args);
...
loadTimesheets = function (exports) {
  var Timesheets = function(storage, settings, responder) {
    ...
    var self = this;
    this.responder.on('receiveMessage', function(username, message, channel) {
      self.receiveMessage(username, message, channel);
    });
  };

  // メッセージを受信する
  Timesheets.prototype.receiveMessage = function(username, message, channel) {
    // 日付は先に処理しておく
    ...
    if(command && this[command[0]]) {
      return this[command[0]](username, message, channel);
    }
  }

  // 出勤
  Timesheets.prototype.actionSignIn = function(username, message, channel) {
    if(this.datetime) {
      var data = this.storage.get(username, this.datetime);
      if(!data.signIn || data.signIn === '-') {
        this.storage.set(username, this.datetime, {signIn: this.datetime});
        this.responder.template(channel, "出勤", username, this.datetimeStr);
      }
      else {
        // 更新の場合は時間を明示する必要がある
        if(!!this.time) {
          // 既存の時間と異なる場合のみ更新（分単位で比較）
          var existingTime = _.isDate(data.signIn) ? DateUtils.format("Y/m/d H:M", data.signIn) : null;
          var newTime = DateUtils.format("Y/m/d H:M", this.datetime);
          if(existingTime !== newTime) {
            this.storage.set(username, this.datetime, {signIn: this.datetime});
            this.responder.template(channel, "出勤更新", username, this.datetimeStr);
          }
        }
      }
    }
  ...
  // 退勤
  Timesheets.prototype.actionSignOut = function(username, message, channel) {
    if(this.datetime) {
      var data = this.storage.get(username, this.datetime);
      if(!data.signOut || data.signOut === '-') {
        this.storage.set(username, this.datetime, {signOut: this.datetime});
        this.responder.template(channel, "退勤", username, this.datetimeStr);
      }
      else {
        // 更新の場合は時間を明示する必要がある
        if(!!this.time) {
          // 既存の時間と異なる場合のみ更新（分単位で比較）
          var existingTime = _.isDate(data.signOut) ? DateUtils.format("Y/m/d H:M", data.signOut) : null;
          var newTime = DateUtils.format("Y/m/d H:M", this.datetime);
          if(existingTime !== newTime) {
            this.storage.set(username, this.datetime, {signOut: this.datetime});
            this.responder.template(channel, "退勤更新", username, this.datetimeStr);
          }
        }
      }
    }
  ...
  // 休暇申請
  Timesheets.prototype.actionOff = function(username, message, channel) {
    if(this.date) {
  ...
  // 休暇取消
  Timesheets.prototype.actionCancelOff = function(username, message, channel) {
    if(this.date) {
      var dateObj = new Date(this.date[0], this.date[1]-1, this.date[2]);
      var data = this.storage.get(username, dateObj);
      if(!data.signOut || data.signOut === '-') {
  ...
  // 出勤中
  Timesheets.prototype.actionWhoIsIn = function(username, message, channel) {
    ...
    if(_.isEmpty(result)) {
      this.responder.template(channel, "出勤なし");
    }
    else {
      this.responder.template(channel, "出勤中", result.sort().join(', '));
    }
  };

  // 休暇中
  Timesheets.prototype.actionWhoIsOff = function(username, message, channel) {
    var dateObj = DateUtils.toDate(DateUtils.now());
    ...
    if(_.isEmpty(result)) {
      this.responder.template(channel, "休暇なし", dateStr);
    }
    else {
      this.responder.template(channel, "休暇中", dateStr, result.sort().join(', '));
    }
  };

  // 出勤していない人にメッセージを送る
  Timesheets.prototype.confirmSignIn = function(username, message, channel) {
    var self = this;
    ...
    if(!_.isEmpty(users)) {
      this.responder.template(channel, "出勤確認", users.sort());
    }
  ...
  // 退勤していない人にメッセージを送る
  Timesheets.prototype.confirmSignOut = function(username, message, channel) {
    var dateObj = DateUtils.toDate(DateUtils.now());
    ...
    if(!_.isEmpty(users)) {
      this.responder.template(channel, "退勤確認", users.sort());
    }
  };
